...
def login(user, password, session_name):
    if synology_is_online():
        api_path = 'auth.cgi'
        data_dict = {'api': 'SYNO.API.Auth', 'version': 2, 'method': 'login', 'account': user,
                     'passwd': password, 'session': session_name, 'format': 'sid'}
        result = synology_call(api_path, data_dict)
        print(result)
        if result['success']:
...
def synology_call(api_path, data_dict):
